...
export class MCoffeeService {
  private dummyRes={"success": true,"data":[{"user_id": 1,"avatar": null,"name": "Juan",
            "surname": "Perez",
            "status_id": 0,
...
//////////////////////////////invitaciones//////////////////////////////////////
  ...
  loadParticipantInvitationsList(coffeeId:number) {
      //console.log('Estoy en load, la id es:' + coffeeId)
         /*   const header = new Headers({ 
      'Content-Type': 'application/json', 
      'Authorization': 'Bearer' + localStorage.getItem('token') 
...
sendInvitation(user_id:number)/*:Observable<any>*/{
    ...
    return this.http.post(environment.apiUrl + 'sendInvitation', 
      { headers: header, body: { user_id : user_id } })
      .map((res: Response) => {
        this.participantListChange(res.json().data.map((participantInvitations: ParticipantInvitations) => new ParticipantInvitations().deserialize(participantInvitations)))
      });*/
...
acceptInvitation(invitation_id:number)/*:Observable<any>*/{
    ...
    return this.http.post(environment.apiUrl + 'acceptInvitation', 
      { headers: header, body: { invitation_id : invitation_id } })
      .map((res: Response) => {
        this.participantListChange(res.json().data.map((participantInvitations: ParticipantInvitations) => new ParticipantInvitations().deserialize(participantInvitations)))
      });*/
